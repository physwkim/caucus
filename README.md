...
The default-model column uses claude CLI's tier *aliases* (`opus` /
`sonnet` / `haiku`) rather than pinned versions like `claude-opus-4-7`,
so caucus follows the latest generation automatically. Override with a
pinned version in `roles.toml` when you need reproducibility.

**Override precedence** (highest first):
1. `--model <id>` on the CLI invocation (e.g. `caucus session new --model opus` to force Opus everywhere for one run, or `--model claude-opus-4-7` to pin a specific version)
2. `model = "..."` in the role's `roles.toml` entry
3. Per-role embedded defaults (the table above)
4. Hard-coded `DEFAULT_MODEL = "opus"` if nothing else matches
